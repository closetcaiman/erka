...
logical · character · numeric · list

**Zad. 9.** (zamknięte) Co da `x[[2]]` dla `x <- list(2, "a", "b", TRUE)`?
lista z „a" i „b" · wektor znakowy z literą „a" · liczba 2

**Zad. 10.** (zamknięte) Co zwróci `rbind(c(1,3,5), c(3,2,10))`?
wektor długości 3 · macierz 3×2 · macierz 2×3
...
`cor(x, y, na.rm = TRUE)` · `cor(x, y, use = "complete.obs")` · `cor(na.omit(x), y)`
